...
- Invocation: When you say **"Run commit.prompt.md"**, **"Run commit"**, or explicitly: **"GitHub Copilot: run commit process"** (optionally with a message), the assistant will execute the full commit workflow non-interactively and without additional confirmation. The assistant will:
  ...
  2. Build a commit message file from your supplied header/body (if provided) or use the staged commit message; then run the validator: `./scripts/operational/validate-commit-and-version.sh <commit-msg-file>`. Abort and report on validation errors.
...
- The assistant will not ask follow-up questions during execution. It will abort and return an actionable error message only when a precondition fails (e.g., missing `JASYPT_PASSWORD` for tests, failing tests, validator failure, or network/Git errors). For missing `GITHUB_TOKEN`, it will skip release creation but continue other steps.
